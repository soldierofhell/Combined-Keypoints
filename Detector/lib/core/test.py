...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
        ...
        if cfg.TEST.MASK_AUG.ENABLED:
            masks,features = im_detect_mask_aug(model, im, boxes, im_scale, blob_conv)
        else:
            masks,features = im_detect_mask(model, im_scale, boxes, blob_conv)
    ...
    if cfg.MODEL.KEYPOINTS_ON and boxes.shape[0] > 0:
        ...
        if cfg.TEST.KPS_AUG.ENABLED:
            ...
            heatmaps,features = im_detect_keypoints(model, im_scale, boxes, blob_conv)
        ...
        cls_keyps = None
    return cls_boxes, cls_segms, cls_keyps,features
...
def im_detect_mask(model, im_scale, boxes, blob_conv):
    ...
    pred_masks,pred_feat = model.module.mask_net(blob_conv, inputs)
...
def im_detect_keypoints(model, im_scale, boxes, blob_conv):
    ...
    pred_heatmaps,pred_feat = model.module.keypoint_net(blob_conv, inputs)
    ...
    return pred_heatmaps,pred_feat
